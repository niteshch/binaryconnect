...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out, binary=True):
        ...
        if binary:
            self.p_y_given_x = T.nnet.softmax(T.dot(input, self.W_bin) + self.b)
        else:
    ...
    def binarize(self):
        ...
        W_binarized = -1 if W>0
                      +1 otherwise
        '''
        return T.cast(T.switch(T.gt(self.W, 0), 1, -1), theano.config.floatX)
    ...
    def hinge(self, y_mat):
        '''
        We calculate Hinge loss based on y predicted and actual output.

        hinge_loss = \sum_{i!=output} p_i + 1 - p_{output}

        :type y_mat: theano.tensor.TensorType
        :param y_mat: corresponds to a matrix that gives for each example the correct label
        '''
        temp = T.sum(T.tensordot(self.p_y_given_x, y_mat, axes=[[1],[1]]), axis=1)
        return T.mean(T.sqr(T.maximum(0., temp)))
    ...
    def errors(self, y):
        ...
        if y.dtype.startswith('int'):
            ...
            # temp_np = theano.shared(numpy.asarray(range(10)))
            # return T.mean(T.neq(temp_np[self.y_pred], y))
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out, W=None, b=None,
                 activation=T.tanh, binary=True, epsilon=1e-4, normalization=True):
        """
        ...
        if binary:
            lin_output = T.dot(input, self.W_bin) + self.b
        else:
        ...
        if normalization:
            self.output = (self.outp - T.mean(self.outp))/T.sqrt(T.var(self.outp) + epsilon)
        else:
...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hiddenLayers, binary=True, normalization=True, eps=1e-4):
        ...
        self.params_bin = sum([x.params_bin for x in self.hiddenLayers], []) + self.logRegressionLayer.params_bin
...
def train_nn(train_model, validate_model, test_model,
    ...
    :param lr_decay: learning decay or not
    """
    ...
    # Array for storing validation losses
    errors_iteration = []
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            # different function call depending on lr_decay
            if lr_decay:
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if verbose:
                    ...
                    errors_iteration.append(this_validation_loss)
    ...
    return ((1-best_validation_loss)*100., (1-test_score)*100., errors_iteration)
